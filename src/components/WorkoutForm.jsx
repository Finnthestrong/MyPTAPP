...
function LocalInput({ initValue, onCommit, className, placeholder, autoFocus }) {
  ...
  return (
    <input
      type="text" value={val} placeholder={placeholder} autoFocus={autoFocus}
      className={className}
      onChange={(e) => setVal(e.target.value)}
      onBlur={(e) => { prev.current = e.target.value; onCommit(e.target.value); }}
    />
  );
}

function LocalTextarea({ initValue, onCommit, className, placeholder, rows }) {
  const [val, setVal] = useState(initValue || "");
  const prev = useRef(initValue);
  if (initValue !== prev.current && initValue === "") {
    prev.current = "";
  ...
  return (
    <textarea
      value={val} placeholder={placeholder} rows={rows || 2}
      className={className}
      onChange={(e) => setVal(e.target.value)}
      onBlur={(e) => { prev.current = e.target.value; onCommit(e.target.value); }}
    />
...
const selectStyles = {
  control: (base, state) => ({
    ...
    minHeight: "40px", fontSize: "14px", cursor: "pointer",
    "&:hover": { borderColor: "#93c5fd" },
  }),
  option: (base, state) => ({
    ...base, fontSize: "14px", padding: "10px 12px",
    backgroundColor: state.isSelected ? "#3b82f6" : state.isFocused ? "#eff6ff" : "white",
    color: state.isSelected ? "white" : "#374151", cursor: "pointer",
  }),
...
const emptyExercise = () => ({
  ...
  drillBrand: "", drillExercise: "", drillCustom: "",
  exerciseNote: "",
  videoUrl: "", feedbackPros: "", feedbackCons: "",
});
...
function Chip({ label, onClear }) {
  return (
    <button type="button" onClick={onClear}
      ...
      <span>{label}</span><span className="opacity-50 ml-0.5">×</span>
    </button>
  );
}
...
function WeightDrillDown({ ex, catalog, onUpdate }) {
  ...
  useEffect(() => {
    if (ex.drillBodyPart !== "기타 부위") { setLocalBodyPart(""); setLocalExercise(""); }
  }, [ex.drillBodyPart]);
  ...
  const brandOpts = !isCustom && ex.drillBodyPart && ex.drillTool === "머신"
  ...
  const exerciseList = (() => {
    ...
    if (ex.drillTool === "머신") {
      ...
      list = catalog.filter((e) => e.region === ex.drillBodyPart && e.equipment_type === "머신" && e.brand === ex.drillBrand).map((e) => e.exercise_name);
    } else {
      list = catalog.filter((e) => e.region === ex.drillBodyPart && e.equipment_type === ex.drillTool && !e.brand).map((e) => e.exercise_name);
    }
    return [...list, "기타"];
  })();

  const showExerciseSelect = !isCustom && ((ex.drillTool && ex.drillTool !== "머신") || (ex.drillTool === "머신" && ex.drillBrand));
    ...
    <div className="space-y-2">
      {(ex.drillBodyPart || ex.drillTool || ex.drillBrand) && (
        <div className="flex flex-wrap gap-1.5 pb-1">
          {ex.drillBodyPart && (
            <Chip label={isCustom && ex.drillCustomBodyPart ? `기타: ${ex.drillCustomBodyPart}` : ex.drillBodyPart} onClear={() => resetFrom("bodyPart")} />
          )}
          {!isCustom && ex.drillTool && <Chip label={ex.drillTool} onClear={() => resetFrom("tool")} />}
          {!isCustom && ex.drillBrand && <Chip label={ex.drillBrand} onClear={() => resetFrom("brand")} />}
        </div>
      )}

      {!ex.drillBodyPart && (
        <Select options={bodyPartOpts} value={null}
          onChange={(opt) => onUpdate({ drillBodyPart: opt?.value || "", drillCustomBodyPart: "", drillTool: "", drillBrand: "", drillExercise: "", drillCustom: "", name: "" })}
          placeholder="① 부위 선택" styles={selectStyles} isSearchable={false} />
      )}

      {isCustom && (
        <div className="space-y-2">
          <LocalInput initValue={ex.drillCustomBodyPart}
            onCommit={(val) => { setLocalBodyPart(val); onUpdate({ drillCustomBodyPart: val }); }}
          ...
          {(localBodyPart.trim() || ex.drillCustomBodyPart.trim()) && !ex.name && (
            <LocalInput initValue={ex.drillExercise !== "기타" ? ex.drillExercise : ex.drillCustom}
              onCommit={(val) => { setLocalExercise(val); onUpdate({ drillExercise: val, name: val }); }}
      ...
      {!isCustom && ex.drillBodyPart && !ex.drillTool && (
        <Select options={toolOpts} value={null}
          onChange={(opt) => onUpdate({ drillTool: opt?.value || "", drillBrand: "", drillExercise: "", drillCustom: "", name: "" })}
          placeholder="② 도구 선택" styles={selectStyles} isSearchable={false} />
      )}

      {!isCustom && ex.drillTool === "머신" && !ex.drillBrand && (
        <Select options={brandOpts} value={null}
          onChange={(opt) => onUpdate({ drillBrand: opt?.value || "", drillExercise: "", drillCustom: "", name: "" })}
          placeholder="③ 브랜드 선택" styles={selectStyles} isSearchable={false} />
      )}

      {showExerciseSelect && !ex.drillExercise && (
        <Select options={toOpt(exerciseList)} value={null}
          onChange={(opt) => { const val = opt?.value || ""; onUpdate({ drillExercise: val, drillCustom: "", name: val !== "기타" ? val : "" }); }}
          placeholder="운동 검색 또는 선택" styles={selectStyles} isSearchable noOptionsMessage={() => "검색 결과 없음"} />
      )}

      {!isCustom && ex.drillExercise === "기타" && (
        <LocalInput initValue={ex.drillCustom}
          onCommit={(val) => onUpdate({ drillCustom: val, name: val })}
          placeholder="운동명 직접 입력"
          className="w-full border border-blue-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" autoFocus />
      )}

      {ex.name && (
        <div className="flex items-center justify-between bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">
          <div>
            {isCustom && ex.drillCustomBodyPart && <p className="text-xs text-blue-400 mb-0.5">{ex.drillCustomBodyPart}</p>}
            <p className="text-sm font-bold text-blue-700">{ex.name}</p>
          </div>
          <button type="button" onClick={() => onUpdate({ drillExercise: "", drillCustom: "", name: "" })}
            className="text-xs text-blue-400 hover:text-red-500 transition-colors ml-3">변경</button>
        </div>
      )}
    </div>
  );
...
export default function WorkoutForm({ onClose, onSave, initialData, isPersonal = false, memberId }) {
  ...
  const [exercises, setExercises] = useState(() => {
    ...
    return initialData.exercises.map((ex) => ({
      ...
      feedbackPros: ex.feedbackPros || "",
      feedbackCons: ex.feedbackCons || "",
    }));
  });
  ...
  useEffect(() => {
    supabase.from("exercise_catalog")
      .select("region, equipment_type, brand, exercise_name")
      .order("region").order("equipment_type").order("exercise_name")
      .then(({ data }) => { if (data) setCatalog(data); });
  }, []);

  const toggleMuscleGroup = (group) =>
    setMuscleGroups((prev) => prev.includes(group) ? prev.filter((g) => g !== group) : [...prev, group]);
  ...
  const setExerciseType = (id, type) =>
    setExercises((prev) => prev.map((ex) =>
      ex.id === id ? { ...ex, type, entries: [emptyEntry()], drillBodyPart: "", drillCustomBodyPart: "", drillTool: "", drillBrand: "", drillExercise: "", drillCustom: "", name: "" } : ex
    ));
  ...
  const addEntry = (exId) =>
    setExercises((prev) => prev.map((ex) => (ex.id === exId ? { ...ex, entries: [...ex.entries, emptyEntry()] } : ex)));
  ...
  const updateEntry = (exId, entryId, field, value) =>
    setExercises((prev) => prev.map((ex) =>
      ex.id === exId ? { ...ex, entries: ex.entries.map((e) => (e.id === entryId ? { ...e, [field]: value } : e)) } : ex
    ));
  ...
  const handleSubmit = (e) => {
    ...
    onSave({ id: initialData?.id ?? Date.now(), date, muscleGroups, exercises: validExercises, photos, note: note.trim(), signature });
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/50">
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-lg max-h-[92vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
          <h2 className="text-base font-bold text-gray-800">{initialData ? "운동 기록 수정" : "운동 기록 추가"}</h2>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1">
          <div className="px-6 py-5 space-y-5">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {MUSCLE_GROUPS.map((group) => {
                  ...
                  return (
                    <button key={group} type="button" onClick={() => toggleMuscleGroup(group)}
                      className={`py-2.5 rounded-xl text-sm font-medium transition-all border-2 ${selected ? "bg-blue-600 border-blue-600 text-white shadow-sm" : "bg-white border-gray-200 text-gray-600 hover:border-blue-300 hover:text-blue-600"}`}>
                      {group}
                    </button>
                  );
                })}
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button type="button" onClick={addExercise} className="text-xs text-blue-600 font-medium hover:text-blue-700">+ 운동 추가</button>
              </div>
                ...
                {exercises.map((ex, idx) => (
                  <div key={ex.id} className="bg-gray-50 rounded-xl p-3 space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {exercises.length > 1 && (
                        <button type="button" onClick={() => removeExercise(ex.id)} className="text-xs text-red-400 hover:text-red-600">삭제</button>
                      )}
                    </div>
                    ...
                    <div className="flex rounded-lg border border-gray-200 overflow-hidden">
                      {[
                        ...
                        <button key={t.value} type="button" onClick={() => setExerciseType(ex.id, t.value)}
                          className={`flex-1 py-1.5 text-xs font-medium transition-colors ${ex.type === t.value ? t.active : "bg-white text-gray-400 hover:bg-gray-50"}`}>
                          {t.label}
                        </button>
                      ))}
                    </div>
                    ...
                    {ex.type === "general" && (
                      <>
                        <WeightDrillDown ex={ex} catalog={catalog} onUpdate={(fields) => updateExercise(ex.id, fields)} />
                        ...
                        <div className="flex items-center justify-between">
                          <button type="button" onClick={() => addEntry(ex.id)} className="text-xs text-blue-500 hover:text-blue-700 font-medium">+ 무게 추가</button>
                          {(() => {
                            const vol = ex.entries.reduce((s, e) => s + (parseFloat(e.weight) || 0) * (parseInt(e.sets) || 0) * (parseInt(e.reps) || 0), 0);
                            return vol > 0 ? <span className="text-xs text-gray-500">총 볼륨: <span className="font-semibold text-blue-600">{vol.toLocaleString()} kg</span></span> : null;
                          })()}
                        </div>
                      </>
                    )}
                    ...
                    {ex.type === "cardio" && (
                      <>
                        <Select options={toOpt(CARDIO_LIST)} value={ex.name ? { value: ex.name, label: ex.name } : null}
                          onChange={(opt) => updateExercise(ex.id, { name: opt?.value || "" })}
                          placeholder="유산소 종류 선택" styles={selectStyles} isSearchable={false} />
                        <div className="rounded-lg border border-gray-200 overflow-hidden bg-white">
                        ...
                        <button type="button" onClick={() => addEntry(ex.id)} className="text-xs text-orange-500 hover:text-orange-700 font-medium">+ 항목 추가</button>
                      </>
                    )}
                    ...
                    {ex.type === "stretch" && (
                      <>
                        <LocalInput initValue={ex.name} onCommit={(val) => updateExercise(ex.id, { name: val })}
                        ...
                        <button type="button" onClick={() => addEntry(ex.id)} className="text-xs text-purple-500 hover:text-purple-700 font-medium">+ 부위 추가</button>
                      </>
                    )}

                    {/* 특이사항 */}
                    <LocalInput initValue={ex.exerciseNote} onCommit={(val) => updateExercise(ex.id, { exerciseNote: val })}
                    ...
                    {!isPersonal && (
                      <div className="border-t border-dashed border-gray-200 pt-3 space-y-2">
                        <p className="text-xs font-medium text-gray-400">수업 피드백 <span className="text-gray-300">(선택)</span></p>
                        ...
                        {/* 잘한 점 */}
                        <LocalTextarea initValue={ex.feedbackPros} onCommit={(val) => updateExercise(ex.id, { feedbackPros: val })}
                          placeholder="✅ 잘한 점 (예: 수축 지점에서 1초 정지 훌륭함)" rows={2}
                          className="w-full border border-blue-100 bg-blue-50 rounded-lg px-3 py-2 text-xs text-gray-700 placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-300 resize-none" />

                        {/* 보완할 점 */}
                        <LocalTextarea initValue={ex.feedbackCons} onCommit={(val) => updateExercise(ex.id, { feedbackCons: val })}
                          placeholder="⚠️ 보완할 점 (예: 이완 시 견갑 고정 필요)" rows={2}
                          className="w-full border border-orange-100 bg-orange-50 rounded-lg px-3 py-2 text-xs text-gray-700 placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-orange-300 resize-none" />
                      </div>
                    )}
                  </div>
                ))}
                  ...
                  {photos.map((p) => (
                    <div key={p.id} className="relative group aspect-square">
                      ...
                      <button type="button" onClick={() => removePhoto(p.id)}
                        className="absolute top-0.5 right-0.5 w-5 h-5 bg-black/60 text-white rounded-full text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">&times;</button>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="flex gap-3 px-6 pb-6 pt-2 shrink-0">
            <button type="button" onClick={onClose}
              className="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">취소</button>
            <button type="submit"
